Map<String, dynamic> akteKelahiranData = {
  "title": "Akte Kelahiran",
  "children": [
    "Formulir (F.2.0.1) di Ketahui Lurah.",
    "Surat Kelahiran dari Dokter/Bidan/Penolong Kelahiran/ Bagi yang Lahir Sebelum Tahun 2006 dpat melampirkan Surat Pernyataan Tanggung Jawab Mutlak (SPTJM).",
    "Surat Nikah atau Akta Perkawinan orang tua yang dilegalisir oleh instansi yang berwenang",
    "Foto copy KTP dan KK / orang tua",
    "Foto copy KTPEL 2 (dua) orang saksi.",
    "Foto copy dokumen imigrasi bagi pemohon WNA yang dilegalisir oleh instansi yang berwenang",
    "Surat Permohonan Pencatatan Kelahiran bagi yang Pelapornya melampaui 60 (Enam Puluh) hari sejak kelahiran.",
    "Denda Keterlambatan (lebih dari 60 hari) adalah Rp.25.000,-"
  ]
};

Map<String, dynamic> akteKematian = {
  "title": "Akta Kematian",
  "children": [
    "Formulir Surat Keterangan Kematian di Ketahui Lurah (F229) Disi Lengkap",
    "Formulir Kematin (F228)",
    "Surat Keterangan Kematian dari Rumah Sakit/Puskesmas Aslu atau foto copy legalisir",
    "Foto Copy Surat Nikah/ Akta Perkawinan (apabila yang meninggal sudah menikah) dilegalisir",
    "Apabila Akta Catatan Sipil belum mencantumkan WNI maka dilengkapi Bukti Pewarganegaraannya.",
    "Foto copy KTP Pelapor",
    "Dokumen Imigrasi bagi WNA",
    "Akta Kelahiran yang belum menikah",
    "2 Foto Copy KTP EL Sebagai Saksi",
    "Pengantar RT/RW",
    "Foto Copy KK dan KTP yang meninggal",
    "Penetapan Pengadilan, Untuk yang Peristiwa lebih dari 10 Tahun"
  ]
};

Map<String, dynamic> kartuIdentitasAnak = {
  "title": "(KIA) Kartu Identitas Anak",
  "children": [
    "Foto Copy Akta Kelahiran Anak",
    "Foto copy Kartu Tanda Penduduk dan Kartu Keluarga Orang Tua",
    "Pas foto anak berwarna ukuran 3 x 4 (2 lembar) untuk anak yang berusia 5 Tahun Keatas."
  ]
};

Map<String, dynamic> perpindahanKeluar = {
  "title": "Perpindahan Keluar",
  "children": [
    "Fotocopy Kartu Tanda Penduduk (KTP)",
    "Fotocopy Kartu Keluarga (KK",
    "Formulir (F-1.23) diketahui lurah",
    "Alamat lengkap daerah tujuan"
  ]
};

Map<String, dynamic> ktpElektronik = {
  "title": "KTP Elektronik",
  "children": [
    {
      "label": "Pemohon Baru",
      "child": [
        "Foto Copy KK atau",
        "Surat Keterangan sudah melakukan perekaman KTP-el"
      ]
    },
    {
      "label": "KTP-el Rusak",
      "child": ["Foto Copy KK", "Upload KTP-el rusak"]
    },
    {
      "label": "KTP-el Hilang",
      "child": ["Foto Copy KK", "Surat Kehilangan KTP dari Kepolisian"]
    },
  ]
};
